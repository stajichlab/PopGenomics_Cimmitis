...
#SBATCH -p short
#SBATCH -J mkPopprTree --out logs/setup_tree_poppr.log
...
do
  ...
  do 
    ...
    if [[ ! -s $tree || ${vcf} -nt $tree ]]; then
	    sbatch -N 1 -n 1 -c 8 --mem 32gb -p short -J popprUPGMA --out logs/make_poppr_$POPNAME.upgma.%A.log --wrap "time Rscript ./scripts/poppr_tree.R --vcf $vcf --tree $tree --method upgma"
    fi
    tree=$TREEDIR/$PREFIX.$POPNAME.$TYPE.poppr.nj.tre
    if [[ ! -s $tree || ${vcf} -nt $tree ]]; then
	    sbatch -N 1 -n 1 -c 8 --mem 32gb -p short -J popprNJ --out logs/make_poppr_$POPNAME.nj.%A.log --wrap "time Rscript ./scripts/poppr_tree.R  --vcf $vcf --tree $tree --method nj"
    fi
  done
done
